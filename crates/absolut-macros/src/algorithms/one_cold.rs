...
pub struct OneColdBuilder {
    table: HashMap<Bytes, Class>,
}

impl Builder for OneColdBuilder {
    ...
    fn build(
        ...
        let mut table_lo = [u8::MAX; LANES];
        let mut table_hi = [u8::MAX; LANES];
        ...
        let variants = self
            ...
            .chain(iter::once((wildcard.name.as_str(), u8::MAX)))
            .map(|(name, value)| {
        ...
        let syntax = quote! {
            ...
            impl ::absolut::OneCold for #ident {
