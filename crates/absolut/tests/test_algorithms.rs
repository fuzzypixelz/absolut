...
pub unsafe fn lookup_one_of_8_neon<const ONE_HOT: bool>(
    input: &[u8; 16],
    lo: &[u8; 16],
    hi: &[u8; 16],
) -> [u8; 16] {
...
unsafe fn lookup_one_of_8_ssse3<const ONE_HOT: bool>(
    input: &[u8; 16],
    lo: &[u8; 16],
    hi: &[u8; 16],
) -> [u8; 16] {
...
fn lookup_one_of_8_fallback<const ONE_HOT: bool>(
    input: &[u8; 16],
    lo: &[u8; 16],
    hi: &[u8; 16],
) -> [u8; 16] {
...
fn lookup_one_of_8<const ONE_HOT: bool>(
    input: &[u8; 16],
    lo: &[u8; 16],
    hi: &[u8; 16],
) -> [u8; 16] {
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        if std::arch::is_x86_feature_detected!("ssse3") {
            unsafe { lookup_one_of_8_ssse3::<ONE_HOT>(input, lo, hi) }
        } else {
            lookup_one_of_8_fallback::<ONE_HOT>(input, lo, hi)
        }
    }
    #[cfg(target_arch = "aarch64")]
    {
        if std::arch::is_aarch64_feature_detected!("neon") {
            unsafe { lookup_one_of_8_neon::<ONE_HOT>(input, lo, hi) }
        } else {
            lookup_one_of_8_fallback::<ONE_HOT>(input, lo, hi)
        }
    }
